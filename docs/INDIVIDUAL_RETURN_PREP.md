...
Trust review collections accept capital-gain items, discount eligibility/application signals, franked distributions, franking credits, streaming and specific-entitlement signals, deed/resolution records, and beneficiary component allocations. They route only to `trust_items`; they never reuse individual investment or CGT rows. Missing resolutions, absent statements, unknown or malformed component amounts, unsupported allocation percentages or bases, alias conflicts, invalid source provenance, and invalid checked-at dates produce evidence rows and stay `Accountant review`. The verified ATO trust-streaming source supports the review context only. TaxMate does not work out a capital gain, apply a discount, determine specific entitlement, allocate components, or decide franking treatment.
