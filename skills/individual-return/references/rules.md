...
General CGT event handling is prep-only. Collect top-level facts or itemized non-crypto/non-rental event rows with event type, asset description, owner or ownership share, acquisition date, disposal date, proceeds, cost base, incidental costs, losses, records, and basic review signals such as mixed/private/business use, exemption, discount, and concession flags. Explicit no-CGT answers without facts should skip the workflow; no-CGT plus facts, missing records, unknown or malformed amounts/dates, ownership uncertainty, item/top-level conflicts, partial item totals, and review flags stay Evidence or `Accountant review`. Preserve valid falsey values such as `0` proceeds, `0` cost base, `0` incidental costs, `0` losses, and `false` exemption, discount, concession, business-use, and private-use flags. Completed CGT rows stay `Accountant review`; supplied totals may reconcile to item totals as prep evidence only. Do not calculate a final capital gain or loss, decide final CGT treatment, fill official ATO forms, or call it copy-ready.
