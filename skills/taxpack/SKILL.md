...
- Review queues and context links must not show blank review items; use row number/status fallback when explanation fields are missing.
...
- Render the runtime-owned handoff contract. Every row and queue item needs labelled facts, a nonblank next action, a verified destination or explicit non-entry/review wording, an explanation, and provenance.
- Never infer a destination from row names, broad topic URLs, or AI target labels. Missing, malformed, conflicting, unsupported, or stale mapping state must fail closed.
...
| Destination mapping is absent or invalid | Show destination-review wording; do not create a mapping in the output layer. |
...
- For guide rows, preserve accountant-review status in the row badge, context index, filters, and review queue even if other fields are stale or conflict.
- For guide rows, keep context links and review queue entries visible even when `tab_text` and rationale fields are blank.
- Render field-level actions for mixed rows so one destination is never implied for unrelated facts.
